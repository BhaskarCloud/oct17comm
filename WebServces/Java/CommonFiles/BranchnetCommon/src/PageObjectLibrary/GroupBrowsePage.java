 ...
 * <br> Methods:</br>
 * <br> searchGroupBrowse : This method is set search Criteria in Group Browse Page</br>
 * @author Aiswarya Srinivasan
 * @since 08-29-2016 
 * </p>
 */
public class GroupBrowsePage extends AccountBrowse_Common{
	...
	public final int INT_PAGEID = 163;
	
	public String[][] allObjects = null;
	
	public GroupBrowsePage(WebDriver driver){
		super(driver);
		this.driver = driver;
	...
	/**
	 * This function is used to get the Account no from the Fixed Income page
	 *
	 ...
	 * @param N/A
	 */
	public boolean navigateToWVGrp_CreateClient() {
		try {
			LPLCoreSync.staticWait(bnHome.lplCoreConstents.MediumInMiliSec);
			//driver.switchTo().frame("shell");
			//driver.switchTo().frame(driver.findElement(By.xpath("//body//iframe[position()=2]")));
			List<WebElement> intGrpRows = driver.findElements(By.xpath("//table[@id='ctl00_cph_searchGrid']//tr"));
			System.out.println(intGrpRows.size()); 
			for(int intRow = 1;intRow<=intGrpRows.size();intRow++){
				//driver.findElement(By.id(strGrpNameId)).click();
				//*[@id="ctl00_cph_searchGrid"]/tbody/tr[5]/td[3]/a
				int intRowPos = intRow + 4;
				driver.findElement(By.xpath("//*[@id='ctl00_cph_searchGrid']/tbody/tr["+intRowPos+"]//a")).click();
				LPLCoreSync.staticWait(bnHome.lplCoreConstents.MediumInMiliSec);
				try{
					driver.switchTo().frame(driver.findElement(By.xpath("//body//iframe[position()=7]")));
					driver.findElement(By.xpath("//tr[@id='ctl00_ContentPlaceHolder1_summary_wealthVisionLinkRow']/td[2]/a[text()='Create Client']")).click();
					break;
				}
				catch(Exception ex){
					//Navigate back to 'Group Browse screen'
					driver.switchTo().defaultContent();
					driver.switchTo().frame("shell");
					driver.findElement(By.id("imgOpenPageManager")).click();
					driver.findElement(By.xpath("//table[@id='tableActivePages']/tr[2]/td[2]")).click();
				}
			}
			
			return true;
		} catch (Exception ex) {
			bnHome.strError = bnHome.strError + ex.getMessage();
			ex.printStackTrace();
			return false;
			}
		}
