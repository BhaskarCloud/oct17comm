 ...
 * </p>
 */
public class AccountBrowse_AggregatePositionsPage_AXA extends AccountBrowse_Common_AXA{
	...
	/** String variables To be used to capture any columns missing in the search results occurred at runtime. */
	...
	public final int INT_PAGEID = 342;
	
	public String[][] allObjects = null;
			
	public AccountBrowse_AggregatePositionsPage_AXA(WebDriver driver){
		super(driver);
		this.driver = driver;
		...
		try {
			...
			if (PageObjectsMap.get("strMatchTypeComboBox").get("ID") != null)
				strValueToMatchTextBox_id = PageObjectsMap.get("strMatchTypeComboBox").get("ID");
			
			if (PageObjectsMap.get("strMatchTypeComboBox").get("ID") != null)
				strSortingCriteriaPanel_id = PageObjectsMap.get("strMatchTypeComboBox").get("ID");
	...
	public boolean setSearchCriteriaAgPositions(String strRepID,String strMatchTypeOption, String strMatchTypeValue){
		try {
			...
			SelectDropDown sdd = new SelectDropDown(driver, By.id(strMatchTypeComboBox_id));
